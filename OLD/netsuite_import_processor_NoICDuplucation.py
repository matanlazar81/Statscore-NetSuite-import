...
def process_csv(csv_file, mapping_data):
    ...
    if subsidiary_col and subsidiary_col in df_ic.columns:
        ...
        # Insert "Subsidiary Line/ DUE TO/ FROM SUBSIDIARY" column right after "Subsidiary Line" with value 5 (xElimination - Parent)
        cols = list(df_ic.columns)
        sub_line_idx = cols.index('Subsidiary Line')
        df_ic.insert(sub_line_idx + 1, 'Subsidiary Line/ DUE TO/ FROM SUBSIDIARY', 5)
    ...
    # === NO DUPLICATION - Keep original IC rows only ===
    # IC rows are kept as-is without creating opposite account duplicates
    print(f"  IC rows kept without duplication: {len(df_ic)} rows")
    df_ic_final = df_ic.copy()
    ...
    print(f"  IC transactions (no duplication): {len(df_ic_final)} rows")
